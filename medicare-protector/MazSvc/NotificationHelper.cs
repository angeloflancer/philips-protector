...
namespace MazSvc
{
    ...
    /// Uses WTSSendMessage API - works on Windows XP and later
    /// </summary>
    public class NotificationHelper : IDisposable
    {
        ...
        public void ShowNotification(string title, string message, NotificationType type)
        {
            try
            {
                ...
                if (activeSessions.Length == 0)
                {
                    ...
                    if (consoleSession != -1 && consoleSession != 0)
                    {
            ...
            try
            {
                if (WTSEnumerateSessions(WTS_CURRENT_SERVER_HANDLE, 0, 1, out pSessionInfo, out sessionCount))
                {
                    ...
                    for (int i = 0; i < sessionCount; i++)
                    {
                        ...
                        // Only include active sessions (State == WTSActive) and not session 0
        ...
        private void SendMessageToSession(int sessionId, string title, string message, NotificationType type)
        {
            try
            {
                // Use MB_TOPMOST for less intrusive notification
                ...
                WTSSendMessage(
                    ...
                    style,
                    5, // Timeout in seconds - auto-dismiss after 5 seconds
                    out response,
